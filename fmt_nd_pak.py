...
Version = 'v1.56 TLOU-only Mesh Exporter v6 (May 13, 2026)'
...
gameName = "TLOU2"             # Default game name
...
def registerNoesisTypes():
	handle = noesis.register("Naughty Dog TLOU PAK", ".pak")
...
def pakCheckType(data):
	...
	if magic == 2681 or magic == 2685:
		return 1
	print("Fatal Error: Unsupported non-TLOU file magic: " + str(hex(magic)))
	return 0
...
def getGameName():
	...
	nameBlob = inName + "|" + outName
	if nameBlob.find("\\pc\\main\\") != -1 or nameBlob.find("tloup1") != -1 or nameBlob.find("t1x") != -1 or nameBlob.find("part i") != -1:
		return "TLOUP1"
	return "TLOU2"
...
def findRootDir(path):
	...
	tlou1Idx = path.find("\\pc\\main\\")
	if tlou1Idx != -1:
		return path[:(tlou1Idx + 8)]
	return path
...
fullGameNames = ["The Last of Us P1", "The Last of Us P2"]
gamesList = ["TLOUP1", "TLOU2"]
...
class openOptionsDialogWindow:
	...
	def createPakWindow(self, width=dialogOptions.width, height=dialogOptions.height):
		...
		if self.create(width, height):
			...
			index = self.noeWnd.createEditBox(5, 65, width-20, 45, dialogOptions.currentDir, self.inputCurrentDirEditBox, False)    
			self.currentDirEditBox = self.noeWnd.getControlByIndex(index)
   
                                       
			index = self.noeWnd.createListBox(5, 120, width-20, 380, self.selectPakListItem, noewin.LBS_NOTIFY | noewin.WS_VSCROLL | noewin.WS_BORDER)    
			self.pakList = self.noeWnd.getControlByIndex(index)
   
			self.noeWnd.createStatic("Files to load:", 5, 505, width-20, 20)
			index = self.noeWnd.createListBox(5, 525, width-20, 150, self.selectLoadListItem, noewin.CBS_DROPDOWNLIST)    
			...
			index = self.noeWnd.createComboBox(width-170, 685, 150, 20, self.selectGameBoxItem, noewin.CBS_DROPDOWNLIST)    
			self.gameBox = self.noeWnd.getControlByIndex(index)
   
			self.noeWnd.createStatic("View:", width-210, 720, 60, 20)
			index = self.noeWnd.createComboBox(width-170, 715, 150, 20, self.selectLocalBoxItem, noewin.CBS_DROPDOWNLIST)    
			self.localBox = self.noeWnd.getControlByIndex(index)
   
			self.noeWnd.createStatic("Scale:", width-215, 750, 60, 20)
			index = self.noeWnd.createEditBox(width-170, 750, 80, 20, str(GlobalScale), self.inputGlobalScaleEditBox, False)    
...
class PakFile:
	...
	def readPointerFixup(self, TP1ZeroCondition=False):
		...
		if offset > 0 or TP1ZeroCondition:
			...
			if not TP1ZeroCondition:
				msg = "ReadAddr not found in PointerFixups at 0x%X. This usually means the exporter is reading the wrong descriptor offset for this PAK layout." % readAddr
				print(msg)
	...
	def writeVRAMImage(self, vramOffset, filepath):
		...
		if rapi.checkFileExists(filepath):
			...
			if True:                                         
				...
				bs.seek(32)
				bs.writeUInt(readUIntAt(bs, bs.tell())+len(imgBytes))                        
   
			bs.seek(vramOffset+40)
			bs.writeUInt(newDataOffset - rawDataStart)            
			bs.seek(vramOffset+48)
			bs.writeUInt(len(imgBytes))          
			bs.seek(vramOffset+84)
			bs.writeUInt(width)           
			bs.seek(vramOffset+88)
			bs.writeUInt(height)            
			bs.seek(vramOffset+80)
			bs.writeUInt(numMips)          
	...
	def loadVRAM(self, vramOffset=0, exTexName=""):
		...
		if bigVramOffset: 
			...
			print("VRAM texture hash found!", fileName, '{:02X}'.format(m_hash), texFileName)                                                                                                                 
	...
	def readPakHeader(self):
		...
		m_hdrSize = bs.readUInt()                     
		m_pakLoginTableIdx = bs.readUInt()                                                 
		m_pakLoginTableOffset = bs.readUInt()                                                                                             
		m_pageCt = bs.readUInt()                                                           
		m_pPakPageEntryTable = bs.readUInt()                                           
		m_numPointerFixUpPages = bs.readUInt()                  
		m_pointerFixUpTableOffset = bs.readUInt()                                         
		m_unk5 = bs.readUInt()                   
		m_unk6 = bs.readUInt()                   
		m_unk7 = bs.readUInt()                   
	...
	def readPak(self):
		...
		if not self.jointOffset and dialogOptions.doLoadBase and dialogOptions.baseSkeleton:                                   
		...
		if self.jointOffset:
			...
			for b in range(boneCount):
				...
				if b in self.boneMap:
					...
					matchedName = boneNames[b].replace("_" + endName, "")                         
					bFound = False
					mat = identity
					if parentList[b][3] != -1 and parentList[b][3] in self.boneMap:                                                                                              
			...
			for i in range(m_numSubMeshDesc):
				...
				for j in range(m_numStreamSource):
					...
					if dialogOptions.isTLOU2 or dialogOptions.isTLOUP1:
						...
						uknInt0 = bs.readUInt()
						qScale = NoeVec4((bs.readFloat(), bs.readFloat(), bs.readFloat(), bs.readFloat()))         
				...
				if not material:
					...
					for j in range(texCount):
						...
						bs.seek(8+8*(dialogOptions.isTLOU2 or dialogOptions.isTLOUP1),1)                                             
						bs.seek(readPointerFixup())
						...
						if texFileName and name.find("01") != -1:
							if not loadedDiffuse and name.find("BaseColor01") != -1:
								...
								if dialogOptions.doConvertTex:
									...
									material.flags |= noesis.NMATFLAG_NORMALMAP_FLIPY                                    
									if name.find("NR") != -1 and texFileName.find("-ao") != -1:                    
						...
						if not loadedNormal and not secondaryNormal and name.find("Normal0") != -1:                                                                      
...
def pakWriteModel(mdl, bs):
	...
	def getExportName(fileName):
		...
		injectMeshName = noesis.userPrompt(noesis.NOEUSERVAL_FILEPATH, "Export TLOU .pak", "Choose a TLOU1/TLOU2 .pak file to inject", injectMeshName, None)
	...
	if not (dialogOptions.isTLOU2 or dialogOptions.isTLOUP1):
		print("ERROR: This TLOU-only exporter supports only TLOU1/TLOU2 PAK layouts. U4/TLL behavior has been removed.")
		return 0
	gameName = "TLOUP1" if dialogOptions.isTLOUP1 else "TLOU2"
	...
	if source.submeshes:
		...
		geoSubmeshPtrAddr = source.geoOffset[0] + source.geoOffset[1] + ResItemPaddingSz + 40
		f.seek(geoSubmeshPtrAddr)
		submeshesAddr = source.readPointerFixup()
		if not submeshesAddr:
			print("ERROR: Could not resolve submesh descriptor pointer at 0x%X" % geoSubmeshPtrAddr)
			return 0
		...
		if not texOnly:
			...
			def addNameVariant(outList, rawName):
				if rawName == None:
					return
				try:
					name = str(rawName)
				...
				if not name:
					return
				name = name.replace("\\", "/").split("/")[-1].split("\x00")[0].strip()
				if not name:
					return
				variants = [name]
				try:
					variants.append(rapi.getExtensionlessName(name))
				except:
					pass
				for v in list(variants):
                                                                          
					variants.append(re.sub(r"\.\d{3}$", "", v))
                                                     
					variants.append(re.sub(r"_\d{3}$", "", v))
                                                    
					variants.append(re.sub(r"^\d{4}_", "", v))
                                                                              
					variants.append(re.sub(r"_(mesh|geometry)$", "", v, flags=re.IGNORECASE))
				for v in variants:
					v = v.strip().lower()
					if v and v not in outList:
						outList.append(v)
   
			def meshNameKeys(mesh):
				outList = []
				addNameVariant(outList, getattr(mesh, "name", None))
				addNameVariant(outList, getattr(mesh, "sourceName", None))
				addNameVariant(outList, getattr(mesh, "matName", None))
				return outList
   
			def submeshNameKeys(smName):
				outList = []
				addNameVariant(outList, smName)
				return outList
   
			fbxMeshNameMap = {}
			fbxMeshKeyList = []
			for mesh in fbxMeshList:
				keys = meshNameKeys(mesh)
				fbxMeshKeyList.append((mesh, keys))
				for key in keys:
					if key not in fbxMeshNameMap:
						fbxMeshNameMap[key] = mesh
   
			def findFbxMeshForSubmesh(sm):
				smKeys = submeshNameKeys(sm.name)
				for key in smKeys:
					mesh = fbxMeshNameMap.get(key)
					if mesh:
						return mesh, key
                                                                                
				matches = []
				for mesh, keys in fbxMeshKeyList:
					for mk in keys:
						for sk in smKeys:
							if len(mk) >= 16 and len(sk) >= 16 and (mk == sk or mk.endswith(sk) or sk.endswith(mk)):
								matches.append((mesh, mk))
								break
						if matches and matches[-1][0] == mesh:
							break
				if len(matches) == 1:
					return matches[0]
                                                                               
				if (dialogOptions.isTLOU2 or dialogOptions.isTLOUP1) and len(fbxMeshList) == 1:
					lod0Submeshes = [x for x in source.submeshes if "lod0" in x.name.lower()]
					if len(lod0Submeshes) == 1 and lod0Submeshes[0] == sm:
						return fbxMeshList[0], "single-fbx-single-lod0"
				return None, None
   
			for i, sm in enumerate(source.submeshes):
				writeMesh, matchedKey = findFbxMeshForSubmesh(sm)
				if writeMesh:
					submeshesFound.append(sm.name + "  <=  " + getattr(writeMesh, "name", "<unnamed>"))
				if not writeMesh:
			...
			if len(submeshesFound) > 0:
				...
				for meshName in submeshesFound: 
					print("    " + meshName)
				print("\nAll other submeshes in this TLOU pak will be left untouched.\n")
			else:
				print("Found no submeshes to inject from FBX, copying original file...")
				print("\nFBX mesh names seen by Noesis:")
				for mesh, keys in fbxMeshKeyList[:50]:
					print("    name=%s sourceName=%s keys=%s" % (str(getattr(mesh, "name", "")), str(getattr(mesh, "sourceName", "")), ", ".join(keys[:6])))
				if len(fbxMeshKeyList) > 50:
					print("    ... %i more FBX meshes" % (len(fbxMeshKeyList) - 50))
				print("\nPAK submesh names available:")
				for sm in source.submeshes[:100]:
					print("    " + sm.name)
				if len(source.submeshes) > 100:
					print("    ... %i more PAK submeshes" % (len(source.submeshes) - 100))
				print("\nRename your Blender object/mesh to one of the PAK submesh names above. Blender .001 suffixes are ignored by this exporter.\n")
			...
			if doWrite:
				...
				newPage = pageCt
				appendOwnerIndex = None
    
				def chooseAppendOwnerIndex(sm):
                                                                        
                                                                     
					candidateAddrs = []
					try:
						for sdesc in sm.streamDescs:
							candidateAddrs.append(sdesc.bufferOffsetAddr)
					except:
						pass
					try:
						candidateAddrs.append(sm.facesOffsetAddr)
					except:
						pass
					try:
						if sm.skinDesc:
							candidateAddrs.append(sm.skinDesc.mapOffsetAddr)
							candidateAddrs.append(sm.skinDesc.weightOffsetAddr)
					except:
						pass
					for addr in candidateAddrs:
						pageId = source.getPointerFixupPage(addr)
						if pageId != None and pageId >= 0 and pageId < len(source.pakPageEntries):
							return source.pakPageEntries[pageId][2]
					return source.pakPageEntries[len(source.pakPageEntries)-1][2]
    
				def writeZeroBytes(outbs, count):
                                                                             
					if count <= 0:
						return
					zeroChunk = b"\x00" * min(count, 1048576)
					remaining = count
					while remaining > 0:
						writeCount = min(remaining, len(zeroChunk))
						outbs.writeBytes(zeroChunk[:writeCount])
				...
				def writeT2UnknownOriginalOrZero(tempbs, f, sd, writeSize, writeMesh):
					...
					try:
						...
						oldSize = writeSize
					f.seek(sd.offset)
					oldBytes = f.readBytes(max(0, oldSize))
					f.seek(oldTell)
     
					if sd.type == 130:
						recordSize = 4
						if not oldBytes or oldBytes == b"\x00" * len(oldBytes):
							msg = "ERROR: source tangent/frame stream is empty/zero for " + writeMesh.name + "; refusing to export an invalid tangent/frame stream"
							print(msg)
							raise RuntimeError(msg)
						if len(oldBytes) >= writeSize:
							tempbs.writeBytes(oldBytes[:writeSize])
						else:
							tempbs.writeBytes(oldBytes)
							lastRecordStart = max(0, len(oldBytes) - recordSize)
							lastRecord = oldBytes[lastRecordStart:lastRecordStart+recordSize]
							if len(lastRecord) != recordSize:
								lastRecord = b"\x7f\x00\x00\x80"
							remaining = writeSize - len(oldBytes)
							while remaining > 0:
								chunk = lastRecord[:min(recordSize, remaining)]
								tempbs.writeBytes(chunk)
								remaining -= len(chunk)
						return
					...
					if len(writeMesh.positions) == sd.numVerts:
						...
						msg = "ERROR: unknown TLOU stream type " + str(sd.type) + " changed vertex count for " + writeMesh.name + "; refusing to zero it"
						print(msg)
						raise RuntimeError(msg)

				missingBoneNames = {}
				missingBonePrintLimit = 20
				...
				for i, meshTuple in enumerate(meshesToInject):
					...
					appendedPositions = appendedWeights = appendedIndices = isModded       
					newPageDataAddr = source.pakPageEntries[len(source.pakPageEntries)-1][0] + source.pakPageEntries[len(source.pakPageEntries)-1][1]
					if appendOwnerIndex == None:
						appendOwnerIndex = chooseAppendOwnerIndex(sm)
						print("New mesh pages will use page owner/index", appendOwnerIndex)
					owningIndex = appendOwnerIndex
					vertOffs = submeshesAddr + 192*i + 136
					foundPositions = foundUVs = foundNormals = 0
					appendedPositions = (len(writeMesh.positions) > sm.numVerts) or appendedPositions                                                                                                   
					...
					for j, sd in enumerate(sm.streamDescs):
						bs.seek(sd.offset)
						streamWriteSize = getT2StreamWriteSize(sd, len(writeMesh.positions))
      
						if appendedPositions and wb.tell() + streamWriteSize > 1048032:                           
						...
						if streamWriteSize < 0 or streamWriteSize > 1048032:
							print("ERROR: Refusing unreasonable stream write size", streamWriteSize, "for", sm.name, "stream", j, "type", sd.type)
							return 0
						bufferStart = tempbs.tell()
						writeZeroBytes(tempbs, streamWriteSize)
						...
						writeUIntAt(bs, sd.bufferOffsetAddr + 8, len(writeMesh.positions))                         
						writeUIntAt(bs, sd.bufferOffsetAddr + 16, bufferEnd-bufferStart)                 
						...
						for v, row in enumerate(weightRows):
							...
							for boneIndex, weight in row:
								try:
									...
									try:
										missingName = mdl.bones[boneIndex].name
									except:
										missingName = str(boneIndex)
									missingBoneNames[missingName] = missingBoneNames.get(missingName, 0) + 1
									if len(missingBoneNames) <= missingBonePrintLimit and missingBoneNames[missingName] == 1:
										print("Bone weight", missingName, "not found in pak skeleton; using bone 0")
									boneID = 0
								if sm.skinDesc.uncompressed:
					...
					if sm.nrmRecalcDesc:
						...
						for n in range(4):
							...
							if not appendedPositions or n == 0:
								writeZeroBytes(tempbs, 2 * len(writeMesh.positions))
				...
				newPageStreams.append(wb)
			if len(missingBoneNames) > missingBonePrintLimit:
				print("Suppressed", len(missingBoneNames) - missingBonePrintLimit, "additional missing bone-name warnings")
			if missingBoneNames:
				print("Total missing bone-name references:", sum(missingBoneNames.values()), "across", len(missingBoneNames), "unique bones")
			...
			if texOnly or noesis.optWasInvoked("-texfolder"):
		...
		if doWrite and didAppend:
			...
			newPageStreams = [p for p in newPageStreams if p.getSize() > 0]
			if len(newPageStreams) == 0:
				didAppend = False
			else:
				...
				if True:
					...
					pageTotalSize = 0x100000
					pageHeaderSize = 20
					pagePayloadCapacity = pageTotalSize - pageHeaderSize          
					oldPftOff = pointerFixupTblOffs
					oldPftDataOff = readUIntAt(bs, oldPftOff + 4)
					if oldPftDataOff < oldPftOff + 12:
						print("ERROR: Pointer-fixup data offset 0x%X precedes PFT header end 0x%X" % (oldPftDataOff, oldPftOff + 12))
						return 0
					origGap = oldPftDataOff - oldPftOff - 12
					totalShift = addAmt
					fileSize = bs.getSize()
					trailerSize = readUIntAt(bs, 0x20)
					trailerStart = fileSize - trailerSize if trailerSize > 0 and trailerSize < fileSize else fileSize
					pageEntryTableOff = readUIntAt(bs, 20)
					oldPageEntries = []
					bs.seek(pageEntryTableOff)
					for p in range(pageCt):
						oldOff = bs.readUInt()
						oldSize = bs.readUInt()
						oldOwner = bs.readUInt()
						oldPageEntries.append((oldOff, oldSize, oldOwner))
					oldLastPageEnd = oldPageEntries[-1][0] + oldPageEntries[-1][1] if len(oldPageEntries) else 0
					appendBase = trailerStart
					if oldLastPageEnd > trailerStart:
						print("ERROR: Last registered page end 0x%X is beyond trailer start 0x%X" % (oldLastPageEnd, trailerStart))
						return 0
					if oldLastPageEnd != trailerStart:
						print("WARNING: Last registered page end 0x%X differs from trailer start 0x%X; appending immediately before trailer." % (oldLastPageEnd, trailerStart))
					newPageStart = appendBase + totalShift
     
                                                                       
					writeUIntAt(bs, 4, readUIntAt(bs, 4) + totalShift)
					writeUIntAt(bs, 16, pageCt + numPages)
					writeUIntAt(bs, 28, oldPftOff + addAmt)
					writeUIntAt(bs, oldPftOff + 4, oldPftDataOff + addAmt)
     
                                                                
					bs.seek(pageEntryTableOff)
					for p in range(pageCt):
						bs.writeUInt(oldPageEntries[p][0] + totalShift)
						bs.seek(8, 1)
     
                                                                       
					for i, wb in enumerate(newPageStreams):
						payloadSize = wb.getSize()
						if payloadSize > pagePayloadCapacity:
							print("ERROR: Replacement page payload 0x%X exceeds TLOU page payload capacity 0x%X" % (payloadSize, pagePayloadCapacity))
							return 0
						newPakPageHeaders.writeUInt(newPageStart + i*pageTotalSize)
						newPakPageHeaders.writeUInt(pageTotalSize)
						newPakPageHeaders.writeUInt(owningIndex)
					...
					bs.seek(0)
					ns = NoeBitStream()
					ns.writeBytes(bs.readBytes(oldPftOff))
					ns.writeBytes(newPakPageHeaders.getBuffer())
					bs.seek(oldPftOff)
					ns.writeBytes(bs.readBytes(trailerStart - oldPftOff))
     
                                                                                
					if ns.tell() != newPageStart:
						print("ERROR: TLOU append layout mismatch: ns=0x%X expected newPageStart=0x%X" % (ns.tell(), newPageStart))
						return 0
     
					zeroChunk = b"\x00" * min(pagePayloadCapacity, 1048576)
					for i, wb in enumerate(newPageStreams):
						payloadSize = wb.getSize()
						ns.writeUInt64(16045690984833335023)
						ns.writeUInt(0)
						ns.writeUInt(pageTotalSize)
						ns.writeUShort(owningIndex)
						ns.writeUShort(0)
						ns.writeBytes(wb.getBuffer())
						padRemain = pagePayloadCapacity - payloadSize
						while padRemain > 0:
							padWrite = min(padRemain, len(zeroChunk))
							ns.writeBytes(zeroChunk[:padWrite])
							padRemain -= padWrite
     
					if trailerStart < fileSize:
						bs.seek(trailerStart)
						ns.writeBytes(bs.readBytes(fileSize - trailerStart))
     
					bs.seek(0)
					bs.writeBytes(ns.getBuffer())
					print("Added", numPages, "full registered TLOU pages before trailer")
